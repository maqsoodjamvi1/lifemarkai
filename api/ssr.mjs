 ...
 * artifact scripts/serve-tanstack.mjs hosts on the VPS. On Vercel we skip the
 * Node req/res bridge entirely: the Node runtime accepts a Web-API handler
 * (Request in, Response out), so this file is just env mirroring + a re-export.
 *
 * vercel.json rewrites every non-static path here; static assets win first
 * because outputDirectory (dist/client) is checked before rewrites.
 *
 * Scope note: this deployment covers SSR + the native API routes. The two
 * routes that proxy to the local AI worker process (/api/ai/chat, /api/ai/agent)
 * and Docker-sandbox previews depend on VPS-local processes and are expected
 * to degrade here — production remains the Coolify deployment; this is the
 * Vercel evaluation target from the adoption plan.
 */
...
if (!entry || typeof entry.fetch !== "function") {
  throw new Error(
    `dist/server/server.js does not export a fetch handler (exports: ${Object.keys(mod).join(", ")})`,
  );
}
