...
W,H=A4
GUTTER=0.85*inch; OUTER=0.6*inch; TOP=0.7*inch; BOTTOM=0.7*inch
c=canvas.Canvas("sw_interior_v2.pdf", pagesize=A4)
page=0
...
def word_page(w):
    ...
    # big word top
    c.setFont("Helvetica-Bold",56); c.setFillColor(DARK); c.drawCentredString(lm+cw/2, H-TOP-1.25*inch, w)
    # traceable: ONE large word per line, sized to fit within content width
    fs=52
    while stringWidth(w, "Helvetica-Bold", fs) > cw-6 and fs>20:
        fs-=2
    c.setFillColor(TRACE)
    yrow=H-TOP-2.5*inch
    for r in range(6):
        yy=yrow-r*0.95*inch
        c.setFont("Helvetica-Bold",fs); c.drawString(lm, yy, w)
        c.setStrokeColor(HexColor("#dddddd")); c.setLineWidth(0.8); c.line(lm, yy-8, W-rm, yy-8)
        c.setStrokeColor(HexColor("#f0f0f0")); c.setDash(2,3); c.line(lm, yy+fs*0.55, W-rm, yy+fs*0.55); c.setDash()
...
for w in WORDS[:94]: word_page(w)
c.showPage(); c.save()
meta={"title":"Sight Words Tracing","pages":page}
json.dump(meta, open("sw_meta2.json","w"))
print(json.dumps(meta,indent=2))
# verify no word overflows
cw_odd=W-GUTTER-OUTER
bad=[w for w in WORDS[:94] if min([fs for fs in [52] ])]
print("content width in:",round(cw_odd/72,2))
