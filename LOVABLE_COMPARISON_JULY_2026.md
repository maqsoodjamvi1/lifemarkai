...
8. **Still open** — SSR-first generated stack (TanStack Start/Next default + real toolchain preview; project-scale effort), structured agent tasks panel (tool-activity cards exist; no rollup UI), custom MCP chat connectors, Paddle provider-account *creation* (option UI exists), SOC 2 (organizational, not code).
