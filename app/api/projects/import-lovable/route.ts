  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (!(zipFile instanceof Blob)) {
        return NextResponse.json({ error: "Attach the Lovable ZIP as form field \"zip\"" }, { status: 400 });
      }
      if (zipFile.size > MAX_ZIP_BYTES) {
        return NextResponse.json({ error: "ZIP too large (max 25 MB)" }, { status: 413 });
      }
    ...
    if (raw.length === 0) {
      return NextResponse.json({ error: "No importable files found" }, { status: 422 });
    }
