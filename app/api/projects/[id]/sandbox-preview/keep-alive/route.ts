...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (!isSandboxEnabled()) {
    return NextResponse.json({ enabled: false, alive: false });
  }
  ...
  if (!sandboxId) {
    return NextResponse.json({ enabled: true, alive: false });
  }

  const provider = getSandboxProvider();
  if (typeof provider.keepAlive !== "function") {
    return NextResponse.json({ enabled: true, alive: true, unsupported: true });
  }

  try {
    const res = await provider.keepAlive(sandboxId, previewUrl ? { previewUrl } : undefined);
    return NextResponse.json({ enabled: true, ...res });
  } catch {
    return NextResponse.json({ enabled: true, alive: false });
  }
}
