/**
 * POST /api/titan/cto  — "Act as CTO"
 *
 * Read-only architectural review across five lenses (architecture, scalability,
 * security, code quality, cloud cost) via lib/ai/titan/orchestrator.ctoReview().
 * Returns prioritized, actionable recommendations + a cost summary.
 *
 * See docs/titan/03-self-healing-cto-testing-security.md §2.
 */
...
import { ctoReview } from "@/lib/ai/titan/orchestrator";
...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitAsync(`titan-cto:${user.id}`, RATE_LIMITS.ai);
