...
export async function POST(req: NextRequest) {
  ...
  if (apply === true) {
    ...
    if (!sql || typeof sql !== "string") {
      return NextResponse.json({ error: "sql required when apply is true" }, { status: 400 });
    }
    ...
    if (!/^\s*create\s+(unique\s+)?index\s/i.test(sql) || sql.split(";").filter((s) => s.trim()).length > 1) {
      return NextResponse.json({ error: "Only a single CREATE INDEX statement can be applied here" }, { status: 400 });
    }
    if (!project.cloud_project_ref || !isManagementConfigured()) {
      return NextResponse.json({ error: "A dedicated managed backend is required to apply indexes" }, { status: 400 });
    }
    const res = await runManagedSql(project.cloud_project_ref, sql);
    if (!res.ok) {
      return NextResponse.json({ error: `Index creation failed: ${res.error ?? "unknown error"}` }, { status: 502 });
    }
    return NextResponse.json({ ok: true, applied: sql });
  }
