...
list + data model hints) — which then feeds the same editor-intelligence/autonomous engine
...
flowchart TB
  ...
  FRAMES --> SPEC
  CRAWL --> SPEC
  SPEC["Normalized Spec\n(PRD + screens + data model + design tokens)"] --> ENGINE["Editor Intelligence + Autonomous engine (docs 01–02)"]
```
...
- Already partly present: **`SCREENSHOT_TO_CODE_SYSTEM_PROMPT`** in
  `lib/ai/system-prompts.ts`. Editor intelligence extends it to detect layout, components,
...
flowchart LR
  ...
  NOTE --> ENGINE["Build with editor-intelligence engine"]
```
...
The normalized `Spec` is stored with the durable `project_ai_initiatives` checkpoint/result payloads (doc 06):
