...
drives the internal editor-intelligence lenses (doc 01) to execute it to completion, pausing only at
...
- **Resumability** — graph + per-task status persist in `project_ai_initiatives.checkpoint`
...
- **Roadmap** — epics/tasks (`project_ai_initiatives.checkpoint`, surfaced in the AI Project Manager, doc 05).
...
- `POST /api/editor-intelligence/initiative` — start (streams waves + per-task status as SSE).
- `POST /api/editor-intelligence/initiative/[id]/pause` · `/resume` · `/approve-gate`.
- `GET  /api/editor-intelligence/initiative/[id]/graph` — live task DAG for the UI.
