...
> Cluster goal: turn Editor Intelligence from a tool into a platform — marketplace, app store,
...
The first three already exist — Editor intelligence adds only commissions + enterprise.

## 4. White-Label Platform

Organizations run their own branded LifemarkAI: their branding, domain, and AI models.
...
re-enter from the first non-`done` wave. This is the recommended path for editor intelligence
...
- **AI Project Manager** — tracks features, bugs, releases, sprints, roadmaps
  automatically from `project_ai_initiatives.checkpoint` + `health_findings`. Optionally syncs to
