  ...
  try {
    ...
    if (action === "validate") {
      ...
      try {
        ...
        if (!res.ok) {
          return Response.json({ valid: false, error: "Invalid credentials" });
        }

        return Response.json({ valid: true });
      } catch {
        return Response.json({ valid: false, error: "Could not reach Supabase URL" });
      }
    }
...
export const Route = createFileRoute("/api/integrations/supabase")({
