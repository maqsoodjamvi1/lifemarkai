...
async function authenticateRequest(req: Request): Promise<{ userId: string; scopes: string[] } | null> {
...
async function handlePOST(req: Request) {
  ...
  if (jsonrpc !== "2.0") {
    return Response.json(rpcErr(id ?? null, -32600, "Invalid JSON-RPC version"), { status: 400 });
  }
  ...
  try {
    switch (method) {
      ...
      case "tools/list":
        return Response.json(rpcOk(id, { tools: TOOLS }));
      ...
      case "tools/call": {
        ...
        if (required && !hasScope(scopes, required)) {
          return Response.json(rpcErr(id, -32002, `API key is missing the '${required}' scope for tool '${name}'`), { status: 403 });
        }
      ...
      case "resources/list":
        ...
        return Response.json(rpcOk(id, { resources: [] }));

      case "ping":
        return Response.json(rpcOk(id, {}));

      default:
        return Response.json(rpcErr(id, -32601, `Method not found: ${method}`), { status: 404 });
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : "Internal error";
    return Response.json(rpcErr(id, -32000, msg), { status: 500 });
  }
}
