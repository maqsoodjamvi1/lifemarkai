...
async function handlePOST(req: Request) {
  ...
  if (!oldSnapshotId || !newSnapshotId) {
    return Response.json({ error: "oldSnapshotId and newSnapshotId required" }, { status: 400 });
  }
  ...
  if (!rows || rows.length !== 2) {
    return Response.json({ error: "Snapshot pair not found" }, { status: 404 });
  }
...
export const Route = createFileRoute("/api/projects/snapshots/compare")({
  server: {
    handlers: {
