...
/**
 * Native /api/projects/db-backup — export/restore project files as a SQL dump.
 *   GET ?projectId=  list · POST create (or {action:"restore"}) · DELETE ?id= remove
 */
export const Route = createFileRoute("/api/projects/db-backup")({
  server: {
    handlers: {
      GET: async ({ request }) => {
      ...
      POST: async ({ request }) => {
        ...
        if (action === "restore") {
          if (!content?.trim()) return Response.json({ error: "content is required for restore" }, { status: 400 });
        ...
        if (!files || files.length === 0) return Response.json({ error: "No files to back up" }, { status: 400 });
        ...
        return Response.json({ ...backup, content: sqlDump }, { status: 201 });
      },
