 ...
 */
async function handlePOST(_req: Request, params: any) {
  const { id } = params;
  ...
  if (files.length === 0) {
    return Response.json({ error: "Project has no files yet — generate something first" }, { status: 400 });
  }
...
export const Route = createFileRoute("/api/projects/$id/generate-knowledge")({
  server: {
    handlers: {
