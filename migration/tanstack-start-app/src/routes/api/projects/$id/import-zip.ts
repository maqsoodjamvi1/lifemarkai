...
async function handlePOST(req: Request, params: any) {
  ...
  if (file.size > 50 * 1024 * 1024) {
    return Response.json({ error: "ZIP file must be under 50 MB" }, { status: 413 });
  }
  if (!file.name.endsWith(".zip")) {
    return Response.json({ error: "File must be a .zip archive" }, { status: 400 });
  }
  ...
  if (entries.length === 0) {
    return Response.json({ error: "ZIP is empty or unreadable" }, { status: 400 });
  }
  ...
  if (toImport.length === 0) {
    return Response.json({ error: "No importable source files found in ZIP" }, { status: 400 });
  }
...
export const Route = createFileRoute("/api/projects/$id/import-zip")({
  server: {
    handlers: {
