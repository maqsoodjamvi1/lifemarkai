    ...
    handlers: {
      GET: async ({ params }) => {
        ...
        const [{ data: config }, { data: subscribers }] = await Promise.all([
          (supabase as any).from("app_monetization").select("*").eq("project_id", params.id).maybeSingle(),
          (supabase as any).from("app_subscriptions").select("subscriber_email,status,trial_end,current_period_end,created_at").eq("project_id", params.id).order("created_at", { ascending: false }),
        ]);
      ...
      POST: async ({ request, params }) => {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return Response.json({ error: "Unauthorized" }, { status: 401 });
        const { data: project } = await (supabase as any).from("projects").select("user_id").eq("id", params.id).single();
