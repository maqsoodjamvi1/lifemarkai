  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const r = await listGroupsOrMembers({ data: { groupId } });
      ...
      POST: async ({ request }) => {
        ...
        const r = await createGroup({ data: body });
        if (r.status === "unauthorized") return unauth();
      ...
      PATCH: async ({ request }) => {
        const body = (await request.json().catch(() => ({}))) as any;
        const r = await updateGroup({ data: body });
        if (r.status === "unauthorized") return unauth();
        if (r.status === "bad_request") return Response.json({ error: r.message }, { status: 400 });
        if (r.status === "error") return Response.json({ error: r.message }, { status: 500 });
      ...
      PUT: async ({ request }) => {
        const body = (await request.json().catch(() => ({}))) as any;
        const r = await setGroupMembership({ data: body });
        if (r.status === "unauthorized") return unauth();
        if (r.status === "bad_request") return Response.json({ error: r.message }, { status: 400 });
      ...
      DELETE: async ({ request }) => {
        ...
        const r = await deleteGroup({ data: { groupId } });
        if (r.status === "unauthorized") return unauth();
        if (r.status === "bad_request") return Response.json({ error: r.message }, { status: 400 });
        return Response.json({ ok: true });
      },
