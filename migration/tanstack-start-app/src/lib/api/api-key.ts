 ...
 * (migration 008). Ported verbatim from the Next app — depends only on the
 ...
 * Returns a discriminated result so callers can turn it into a Response.
 */
export async function authenticateApiRequest(
  ...
  if (!token) {
    return { ok: false, status: 401, error: "Missing API key. Send 'Authorization: Bearer lmk_…'." };
  }
