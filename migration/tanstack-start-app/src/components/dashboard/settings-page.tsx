...
export function SettingsPage({ user, profile }: SettingsPageProps) {
  ...
  // Next's `router.refresh()` had no `navigate` equivalent, so the migration
  // codemod left the calls below referencing an undeclared `router`.
  // `router.invalidate()` is the TanStack equivalent (re-runs route loaders).
