@echo off
REM Push the preview process-isolation fix (no clone; refs only).
setlocal
set "HERE=%~dp0"
set "BUNDLE=%HERE%lifemark-preview-isolation.bundle"
set "LOG=%HERE%push41-result.txt"
set "REPO=D:\Projects\lifemarkai"
set "BRANCH=codex/security-hardening"
set "TMPREF=refs/lm/previewiso"

> "%LOG%" 2>&1 (
  echo === LifemarkAI push41: preview gets its own process ===
