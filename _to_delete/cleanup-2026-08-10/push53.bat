...
> "%LOG%" 2>&1 (
  ...
  echo.
  echo [5/6] Committing ...
  git add "%APP%/src/components/editor/chat-panel.tsx" "%APP%/src/components/editor/lovable/prompt-queue.tsx"
  git -c core.hooksPath=NUL commit -m "fix(chat): a queued message with a text attachment threw on save and lost the file" -m "QueueItem.attachedText was typed string, but every producer pushes the composer's { name, content } object. Two consequences, both silent: the persist path called .slice() on an object, throwing inside the debounced save so the whole queue failed to persist; and the load path typeof-checked for \"string\" and therefore nulled the attachment on every reload." -m "Typed the field as the object it always was, taught the loader to accept both shapes so anything already stored as a bare string still opens, and fixed the persist to slice the content rather than the object. The union narrowing added in 4cc434e was treating a symptom downstream of both faults and is now unreachable, so it is removed."
