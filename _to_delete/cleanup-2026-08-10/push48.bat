@echo off
REM deploy48 - merge codex/security-hardening into master and let Coolify rebuild.
REM Gated: type-check and tests must pass, or master is rolled back and nothing ships.
setlocal
set "HERE=%~dp0"
set "LOG=%HERE%deploy48-result.txt"
set "REPO=D:\Projects\lifemarkai"
set "BRANCH=codex/security-hardening"

> "%LOG%" 2>&1 (
  echo === LifemarkAI deploy48: merge %BRANCH% into master ===
  cd /d "%REPO%"
  ...
  echo.
  echo [5/9] Type check ^(this is what the Coolify build runs - catching it here saves a failed deploy^) ...
  call npm run type-check
  if errorlevel 1 (
    echo.
    echo ERROR: type-check failed. Rolling master back to origin/master ...
