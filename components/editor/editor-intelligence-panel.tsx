...
export function EditorIntelligencePanel({ projectId, onSendPromptToChat }: EditorIntelligencePanelProps) {
  ...
  async function runBuild(resumeRunId?: string) {
    ...
    try {
      const res = await fetch("/api/editor-intelligence/initiative", {
        ...
        body: JSON.stringify({ projectId, goal, runId: resumeRunId, seedAgents: false }),
      });
      if (!res.ok || !res.body) {
      ...
      while (true) {
        ...
        for (const frame of frames) {
          ...
          switch (ev.type) {
            case "initiative_run": {
              const id = typeof ev.initiativeId === "string" ? ev.initiativeId : null;
              if (id) {
                setActiveRunId(id);
                localStorage.setItem(`lifemark:editor-intelligence:${projectId}:run`, id);
              }
              log(`${ev.resumed ? "Resumed" : "Started"} durable run ${id?.slice(0, 8) ?? ""}`);
              break;
            }
            ...
            case "done":
              log(`Done — ${(ev.filesChanged as unknown[] ?? []).length} files, ${Number(ev.creditsUsed ?? 0)} credits`);
              localStorage.removeItem(`lifemark:editor-intelligence:${projectId}:run`);
              setActiveRunId(null);
              break;
          }
        }
  ...
  useEffect(() => {
    ...
    const storedRunId = localStorage.getItem(`lifemark:editor-intelligence:${projectId}:run`);
      ...
      <div className="border-t border-border p-3 space-y-2">
        ...
        {activeRunId && !building && (
          <Button
            ...
            disabled={!state || state.agents.length === 0}
          >
