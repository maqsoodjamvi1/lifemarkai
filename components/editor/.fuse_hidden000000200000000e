...
const CONNECTORS: Connector[] = [
  ...
  },
  {
    ...
    npm: ["jira.js"],
    envVars: [
      { key: "ATLASSIAN_HOST", description: "Your Atlassian site host", example: "yourorg.atlassian.net" },
      { key: "ATLASSIAN_EMAIL", description: "Account email for API auth", example: "you@company.com" },
      { key: "ATLASSIAN_API_TOKEN", description: "Atlassian API token", example: "ATATT..." },
      { key: "JIRA_PROJECT_KEY", description: "Default Jira project key", example: "ENG" },
    ],
    setupSteps: [
      "Go to id.atlassian.com/manage-profile/security/api-tokens and create a token",
      "Note your site host (e.g. yourorg.atlassian.net) and project key",
      "Click 'Apply to project' to add Jira + Confluence integration",
    ],
    integrationPrompt: "Integrate Atlassian Jira into this app. Install jira.js. Use ATLASSIAN_HOST, ATLASSIAN_EMAIL, ATLASSIAN_API_TOKEN, and JIRA_PROJECT_KEY env vars. Create lib/jira.ts with a configured Version3Client. Add API routes: GET /api/jira/issues (list issues for the project), POST /api/jira/issues (create issue with summary, description, issue type), GET /api/jira/issues/[key] (issue detail). Add a /jira page with a filterable issue table and a create-issue form. Optionally add GET /api/confluence/pages to fetch Confluence pages for docs context.",
  },
  {
    id: "inngest",
    name: "Inngest",
    description: "Background jobs, scheduled tasks, and durable workflows",
    category: "devtools",
    icon: "⏱️",
    color: "bg-violet-500/10 border-violet-500/20",
    ...
    npm: ["inngest"],
    envVars: [
      { key: "INNGEST_EVENT_KEY", description: "Inngest event key", example: "..." },
      { key: "INNGEST_SIGNING_KEY", description: "Inngest signing key", example: "signkey-prod-..." },
    ],
    setupSteps: [
      "Sign up at inngest.com and create an app",
      "Copy Event Key and Signing Key from the Inngest dashboard",
      "Click 'Apply to project' to add durable workflows",
    ],
  ...
  },
  {
    id: "snowflake",
    name: "Snowflake",
    description: "Query data and run SQL against your Snowflake warehouse",
    category: "database",
    icon: "❄️",
    ...
    npm: ["snowflake-sdk"],
    envVars: [
      { key: "SNOWFLAKE_ACCOUNT", description: "Account identifier", example: "xy12345.us-east-1" },
      { key: "SNOWFLAKE_USERNAME", description: "Username", example: "MY_USER" },
      { key: "SNOWFLAKE_PASSWORD", description: "Password", example: "..." },
      { key: "SNOWFLAKE_DATABASE", description: "Database", example: "MY_DB" },
      { key: "SNOWFLAKE_WAREHOUSE", description: "Warehouse", example: "COMPUTE_WH" },
    ],
    setupSteps: [
      "Create a Snowflake user with read access to your database",
      "Copy account identifier, database, and warehouse names",
      "Click 'Apply to project' to add Snowflake analytics",
    ],
    integrationPrompt: "Integrate Snowflake into this app. Install snowflake-sdk. Use SNOWFLAKE_ACCOUNT, SNOWFLAKE_USERNAME, SNOWFLAKE_PASSWORD, SNOWFLAKE_DATABASE, SNOWFLAKE_WAREHOUSE env vars. Create lib/snowflake.ts with a connection pool. Add GET /api/snowflake/query?sql= (read-only SELECT) and POST /api/snowflake/query for parameterized queries. Build an /analytics page with a SQL editor, run button, and results table. Block DDL/DML — only allow SELECT statements.",
  },
  {
    id: "bigquery",
    name: "BigQuery",
    description: "Query Google BigQuery datasets and build analytics dashboards",
    category: "database",
    icon: "📊",
    color: "bg-blue-500/10 border-blue-500/20",
    ...
    envVars: [
      { key: "BIGQUERY_PROJECT_ID", description: "GCP project ID", example: "my-gcp-project" },
      { key: "BIGQUERY_SERVICE_ACCOUNT_JSON", description: "Service account JSON (single line)", example: "{\"type\":\"service_account\"…}" },
    ],
    setupSteps: [
      "Create a GCP service account with BigQuery Data Viewer role",
      "Download the JSON key and minify to one line",
      "Click 'Apply to project' to add BigQuery integration",
    ],
    integrationPrompt: "Integrate Google BigQuery into this app. Install @google-cloud/bigquery. Use BIGQUERY_PROJECT_ID and BIGQUERY_SERVICE_ACCOUNT_JSON env vars. Create lib/bigquery.ts client. Add GET /api/bigquery/datasets and POST /api/bigquery/query with { sql, params? }. Build an /analytics page listing datasets and running read-only SQL with paginated results. Validate queries are SELECT-only server-side.",
  },
  {
    id: "brevo",
    name: "Brevo",
    description: "Transactional and marketing email with contacts and lists",
    category: "communication",
    icon: "📧",
    ...
    npm: ["@getbrevo/brevo"],
    envVars: [
      { key: "BREVO_API_KEY", description: "Brevo API key", example: "xkeysib-..." },
    ],
    setupSteps: [
      "Sign up at brevo.com and create an API key",
      "Verify your sender domain",
      "Click 'Apply to project' to add Brevo email",
    ],
    integrationPrompt: "Integrate Brevo (Sendinblue) email into this app. Install @getbrevo/brevo. Use BREVO_API_KEY. Create lib/brevo.ts with sendTransactionalEmail and upsertContact helpers. Add POST /api/email/send and POST /api/contacts/sync routes. Wire welcome emails on signup and add a /admin/contacts page listing synced contacts.",
  },
  {
    id: "mailgun",
    name: "Mailgun",
    description: "Transactional email with delivery tracking and webhooks",
    category: "communication",
    icon: "🔫",
    color: "bg-orange-500/10 border-orange-500/20",
    docsUrl: "https://documentation.mailgun.com/",
    npm: ["mailgun.js", "form-data"],
    envVars: [
      { key: "MAILGUN_API_KEY", description: "Mailgun API key", example: "key-..." },
      { key: "MAILGUN_DOMAIN", description: "Sending domain", example: "mg.yourdomain.com" },
    ],
    setupSteps: [
      "Sign up at mailgun.com and verify your domain",
      "Copy the API key and domain from the dashboard",
      "Click 'Apply to project' to add Mailgun email",
    ],
    integrationPrompt: "Integrate Mailgun into this app. Install mailgun.js and form-data. Use MAILGUN_API_KEY and MAILGUN_DOMAIN env vars. Create lib/mailgun.ts client. Add POST /api/email/send route and POST /api/webhooks/mailgun for delivery events (delivered, bounced, complained). Build email templates for welcome and password reset. Log delivery status to a emails_log table.",
  },
  {
    id: "plaid",
    name: "Plaid",
