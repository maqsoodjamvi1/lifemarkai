...
export function suggestedStartingTier(
  ...
  for (const a of attempts) {
    if (a.fullyResolved) continue; // this tier DID work here — do not skip it
    const tier = tierOfModel(a.model);
    if (tier === null) continue;
    // Only a tier that failed AND made no progress is evidence against itself.
    // One that resolved something was working; it simply needed another turn.
    if (a.resolved === 0 || a.madeWorse) floor = Math.max(floor, tier + 1);
  }
...
export function buildPriorAttemptsBlock(attempts: readonly PriorAttempt[]): string {
  ...
  return [
    "",
    "This exact failure has been attempted before:",
    ...lines,
    guidance,
    "",
  ].join("\n");
...
export async function lookupPriorAttempts(
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(
        "model, round, resolved, introduced, fully_resolved, made_worse, files_written, sample_label, project_id",
      )
    ...
    return toPriorAttempts(data as RepairOutcomeRow[], opts.projectId);
  } catch {
    return [];
  }
}
