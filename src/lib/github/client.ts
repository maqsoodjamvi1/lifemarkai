...
import { resolveGitHubApiBase } from "./host";

/** Token plus optional GitHub Enterprise Server API base (`https://host/api/v3`). */
export type GitHubAuth = string | { token: string; apiBase?: string | null };

export function createGitHubClient(auth: GitHubAuth) {
  const resolved =
    typeof auth === "string" ? { token: auth, apiBase: undefined as string | undefined } : auth;
  const baseUrl = resolveGitHubApiBase(resolved.apiBase);
  return new Octokit({
    auth: resolved.token,
    ...(baseUrl ? { baseUrl } : {}),
  });
...
export async function listRepos(auth: GitHubAuth): Promise<GitHubRepo[]> {
...
export async function createRepo(auth: GitHubAuth, name: string, description?: string): Promise<GitHubRepo> {
  const octokit = createGitHubClient(auth);
...
export async function deleteWebhook(auth: GitHubAuth, repo: string, hookId: number): Promise<void> {
  try {
    const octokit = createGitHubClient(auth);
    const [owner, repoName] = repo.split("/");
