...
async function handlePOST(req: Request) {
  ...
  if (!REFACTOR_PROMPTS[refactorType]) return Response.json({ error: "Unknown refactor type" }, { status: 400 });
  ...
  try {
    ...
    return Response.json({ error: "Refactor failed" }, { status: 502 });
  }
}


export const Route = createFileRoute("/api/ai/refactor")({
  server: {
    handlers: {
