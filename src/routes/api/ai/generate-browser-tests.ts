...
async function handlePOST(req: Request) {
  ...
  if (!projectId || !projectName) {
    return Response.json({ error: "projectId and projectName required" }, { status: 400 });
  }
...
Return only the JSON object.`;
  ...
  try {
    ...
    return Response.json({ file, summary: parsed.summary });
  } catch (err) {
...
export const Route = createFileRoute("/api/ai/generate-browser-tests")({
  server: {
    handlers: {
