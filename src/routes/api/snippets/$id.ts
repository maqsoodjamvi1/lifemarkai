  ...
  server: {
    handlers: {
      PATCH: async ({ request, params }) => {
        ...
        const patch: Database["public"]["Tables"]["prompt_snippets"]["Update"] = {};
        ...
        const { data, error } = await supabase.from("prompt_snippets").update(patch).eq("id", params.id).eq("user_id", user.id).select("*").single();
      ...
      DELETE: async ({ params }) => {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return Response.json({ error: "Unauthorized" }, { status: 401 });
        const { error } = await supabase.from("prompt_snippets").delete().eq("id", params.id).eq("user_id", user.id);
        if (error) return Response.json({ error: error.message }, { status: 500 });
      ...
      POST: async ({ params }) => {
        ...
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return Response.json({ error: "Unauthorized" }, { status: 401 });
        const { error } = await supabase.rpc("increment_snippet_use_count", { snippet_id: params.id });
        if (error) {
          const { data: existing } = await supabase.from("prompt_snippets").select("use_count").eq("id", params.id).maybeSingle();
          if (existing) await supabase.from("prompt_snippets").update({ use_count: (existing.use_count ?? 0) + 1 }).eq("id", params.id);
        }
        return new Response(null, { status: 204 });
      },
    },
