...
interface DbResult<T = unknown> { data: T | null; error: DbError | null; }
...
interface ProjectRow { id: string; name: string | null; environment?: string | null; }
interface ProjectFileRow { path?: string | null; content?: string | null; }
interface AgentIdRow { id?: string | null; }
    ...
    async start(controller) {
      ...
      const runQaVerification = async (filesChanged: string[]): Promise<SelfVerifyResult | null> => {
        ...
        if (Date.now() - routeStartedAt > VERIFY_TIME_CUTOFF_MS) {
          ...
          send(skipped);
          await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event: skipped }).catch(() => {});
          return null;
        }
        try {
          const verification = await runSelfVerification({
            ...
            emit: (status) => {
              ...
              send(progress);
              void appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event: progress }).catch(() => {});
          ...
          send(verifyEvent);
          await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event: verifyEvent }).catch(() => {});
          await recordEditorIntelligenceBuild({
            supabase, projectId, projectName: project?.name ?? null, source: "editor-intelligence", mode: "initiative",
            prompt: initiativeRun.goal ?? requestedGoal, filesChanged, verification,
          });
      ...
      try {
        const runStartEvent = { type: "initiative_run", initiativeId: initiativeRun.id, status: initiativeRun.status, resumed: !!existingRun };
        send(runStartEvent);
        await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event: runStartEvent });
        ...
        for await (const event of runInitiative({
          ...
          onCheckpoint: (checkpoint) => updateEditorInitiativeCheckpoint({ supabase, initiativeId: initiativeRun.id, checkpoint }),
          environment: "test",
          ...
          onVerify: async ({ filesChanged }) => {
            if (!filesChanged.length) return { ok: true, summary: "no file changes to verify" };
            const verification = await runQaVerification(filesChanged);
            if (!verification) return { ok: true, summary: "verify unavailable" };
            return {
              ok: verification.passed,
              summary: verification.passed
                ? `self-verify passed (${verification.engine}, ${verification.rounds} rounds)`
                : `self-verify failed: ${(verification.errors || []).slice(0, 3).join("; ")}`,
              fixedFiles: verification.fixedFiles?.map((f) => ({ path: f.path, content: f.content })),
            };
          },
        })) {
          if (event.type === "done") {
            ...
            const verification =
              event.verification && typeof event.verification === "object"
                ? (event.verification as SelfVerifyResult)
                : await runQaVerification(event.filesChanged);
            const doneEvent: EditorIntelligenceEvent = verification
              ? {
                  ...
                  filesChanged: [
                    ...new Set([
                      ...event.filesChanged,
                      ...((verification as SelfVerifyResult).fixedFiles?.map((f) => f.path) ?? []),
                    ]),
                  ],
                  verification,
                }
              : event;
            send(doneEvent);
            await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event: doneEvent });
            continue;
          }

          send(event);
          await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event });
        ...
        send(event);
        await appendEditorInitiativeEvent({ supabase, initiativeId: initiativeRun.id, projectId, event }).catch(() => {});
        await failEditorInitiativeRun({ supabase, initiativeId: initiativeRun.id, error: message }).catch(() => {});
      }
