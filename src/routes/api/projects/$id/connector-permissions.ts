...
async function handlePOST(req: Request, params: any) {
  ...
  if (decision !== "once" && decision !== "always" && decision !== "never") {
    return Response.json({ error: "decision must be once | always | never" }, { status: 400 });
  }
  ...
  return Response.json({ ok: true, connector: id, decision });
}


export const Route = createFileRoute("/api/projects/$id/connector-permissions")({
  server: {
    handlers: {
