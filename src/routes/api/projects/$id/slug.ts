  ...
  server: {
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const { data: existing } = await supabase.from("projects").select("id").eq("app_slug", slug).neq("id", params.id).maybeSingle();
      ...
      PATCH: async ({ request, params }) => {
        ...
        const { data, error } = await supabase.from("projects").update({ app_slug: slug }).eq("id", params.id).select("id, name, app_slug").single();
        if (error) {
