...
const perks = [
  ...
  "Deploy to lifemarkai.app subdomain",
...
function SignupContent() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.session) {
        window.location.replace(getRedirectDestination());
      } else {
        setEmailSent(true);
      }
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-4xl grid md:grid-cols-2 gap-8 items-center">
        ...
        >
          ...
          <p className="text-muted-foreground mb-8">
            Describe your app in plain English. Get production-ready React code,
            deployed in seconds — no coding required.
          </p>
        ...
        >
          ...
          <div className="bg-card border border-border rounded-2xl p-8 shadow-xl">
            ...
            <div className="space-y-3">
              <Button variant="outline" className="w-full h-11 gap-2" onClick={() => handleOAuth("github")} disabled={!!oauthLoading}>
                {oauthLoading === "github" ? <Loader2 className="w-4 h-4 animate-spin" /> : <Github className="w-4 h-4" />}
                Continue with GitHub
              </Button>
              <Button variant="outline" className="w-full h-11 gap-2" onClick={() => handleOAuth("google")} disabled={!!oauthLoading}>
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="password" type="password" placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} className="h-11" />
              </div>
              <Button type="submit" className="w-full h-11" disabled={loading}>
                {loading ? <><Loader2 className="w-4 h-4 animate-spin mr-2" /> Creating account...</> : <><Mail className="w-4 h-4 mr-2" /> Create account</>}
              </Button>
            </form>
