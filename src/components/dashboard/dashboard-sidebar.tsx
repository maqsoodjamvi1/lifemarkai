  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if ((e.metaKey || e.ctrlKey) && e.key === "n" && !e.shiftKey) {
        e.preventDefault();
        navigate({ to: "/dashboard", search: { new: "true" } as never });
      }
    }
  ...
  return (
    <aside className="w-60 flex flex-col border-r border-border bg-sidebar h-full">
      ...
      <div className="h-16 flex items-center px-4 border-b border-border">
        <Link to="/dashboard" className="flex items-center gap-2 font-bold flex-1">
      ...
      <div className="px-3 py-3 border-b border-border">
        <Button
          ...
          onClick={() => navigate({ to: "/dashboard", search: { new: "true" } as never })}
          size="sm"
          title="New Project (⌘N)"
        >
