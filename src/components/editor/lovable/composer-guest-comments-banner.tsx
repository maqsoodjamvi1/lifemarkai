  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          <MessageSquare className="w-4 h-4 text-sky-700 dark:text-sky-300 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-sky-900 dark:text-sky-100">
              {count} guest comment{count === 1 ? "" : "s"} waiting
            </p>
            <p className="text-[10px] text-sky-700/70 dark:text-sky-300/70 mt-0.5">
          ...
          {onFixWithAI && (
            <button
              type="button"
              onClick={onFixWithAI}
              className="shrink-0 rounded-lg border border-sky-500/40 bg-sky-500/15 hover:bg-sky-500/25 text-sky-900 dark:text-sky-100 text-[11px] font-medium px-3 py-1.5 transition-colors"
            >
              Fix with AI
            </button>
          )}
