...
export function LovableStreamingFilesCard({ paths, maxVisible = 8, onOpenPath }: LovableStreamingFilesCardProps) {
      ...
      <div className="px-3 py-2 flex flex-wrap gap-1.5">
        {paths.slice(0, maxVisible).map((path) => (
          <button
            ...
            className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-background/70 text-[var(--fg-tertiary)] border border-[color:var(--border-default)] hover:text-foreground hover:border-border cursor-pointer"
          >
            {path.split("/").pop() ?? path}
          </button>
        ))}
