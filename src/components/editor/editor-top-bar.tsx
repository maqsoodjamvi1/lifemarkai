...
export function EditorTopBar({
  ...
  // TanStack's router instance. Next's `router.refresh()` (re-run server data for
  // the current route) has no `navigate` equivalent — the closest is
  // `router.invalidate()`, which re-runs route loaders. See the visibility
  // toggle below.
  ...
  async function handleTogglePublic() {
    ...
    try {
      ...
      // Was `router.refresh()` under Next; TanStack's equivalent re-runs loaders.
        ...
        >
          ...
          >
            ...
            <DropdownMenuContent align="start" className="w-56">
              ...
              <DropdownMenuItem onClick={() => navigate({ to: "/dashboard" })} className="text-xs gap-2">
