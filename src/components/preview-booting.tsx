...
import {
  ...
  type BootRetryState,
} from "@/lib/preview/preview-boot-retry";
...
const WINDOW_NAME_PREFIX = "lm-preview-boot:";
 ...
 */
function decodeState(raw: string | null, now: number): BootRetryState | null {
  if (!raw) return null;
  try {
    return restoreBootRetryState(JSON.parse(raw), now);
  } catch {
    return null;
  }
}

function readState(): BootRetryState {
  const now = Date.now();
  const fresh = { attempt: 0, since: now, updatedAt: now };
  try {
    const stored = decodeState(window.sessionStorage.getItem(storageKey()), now);
    if (stored) return stored;
  } catch {
    /* fall through to the browsing-context fallback */
  }

  try {
    const nameState = window.name.startsWith(WINDOW_NAME_PREFIX)
      ? decodeState(window.name.slice(WINDOW_NAME_PREFIX.length), now)
      : null;
    return nameState ?? fresh;
  } catch {
    return fresh;
  }
}

function writeState(state: BootRetryState): void {
  const encoded = JSON.stringify(state);
  try {
    window.sessionStorage.setItem(storageKey(), encoded);
    return;
  } catch {
    // window.name survives a reload even in sandboxed/opaque-origin frames
    // where Web Storage throws. It is scoped to this browsing context.
    try {
      window.name = `${WINDOW_NAME_PREFIX}${encoded}`;
    } catch {
      /* the page still reloads; persistence is best-effort in this rare case */
    }
  }
}
